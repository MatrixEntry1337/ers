...
public class Facade {
	...
	public Reim createReim(User author, int amount, int typeId, String description) throws SQLException{
		Connection conn = getConnection();
		ReimDAO dao = ReimFactory.newReimDAO(conn);
		Reim reim = dao.createReim(author, amount, typeId, description);
		conn.commit();
		conn.close();
		return reim;
	}
